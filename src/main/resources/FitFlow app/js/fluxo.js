...
const CONNS_MB = [
  { from:'cb-server',  fs:'bottom', to:'cb-treinodao-mb', ts:'top',    label:'chama' },
  { from:'cb-server',  fs:'bottom', to:'cb-alunodao-mb',  ts:'top',    label:'chama' },
  { from:'cb-server',  fs:'bottom', to:'cb-userdao-mb',   ts:'top',    label:'chama' },
  { from:'cb-treinodao-mb',fs:'bottom',to:'cb-jpa-mb',    ts:'top',    label:'JPAUtil' },
  { from:'cb-jpa-mb',      fs:'right', to:'cb-db-mb',     ts:'left',   label:'JDBC' }
];
...
function orthogonalPath(a, b, fs, ts, index) {
  ...
  // Base Y um pouco abaixo da caixa de origem
  let baseMidY = a.y + 20;
...
function mapEvent(c,a){
  ...
  if(s.includes('treinodao')){
    ...
    return {box: currentTab==='desktop' ? 'cb-treinodao':'cb-treinodao-mb',method:m,zone: currentTab==='desktop' ? 'dao':'dao-mb'};
  }
  if(s.includes('jpa')||s.includes('entitymanager')) return {box: currentTab==='desktop' ? 'cb-jpa' : 'cb-jpa-mb',method:'mt-getEM',zone:currentTab==='desktop'?'infra':'infra-mb'};
  if(s.includes('postgres')||s.includes('db')) return {box: currentTab==='desktop' ? 'cb-db' : 'cb-db-mb',method:'mt-select',zone:currentTab==='desktop'?'infra':'infra-mb'};
  return null;
}
...
// ─── SSE + EVENT QUEUE ──────────────────
let evCount=0, es=null, lastBox=null;
...
function renderEvent(d){
  ...
  $all('.uml.active').forEach(b=>{b.classList.remove('active');b.classList.add('done')});
